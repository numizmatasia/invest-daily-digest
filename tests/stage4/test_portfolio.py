...
def test_current_repository_portfolio_format_is_supported(portfolio_raw):
    ...
    assert snapshot.freshness_status == "UNVERIFIED"
    assert "PORTFOLIO_FRESHNESS_UNVERIFIED_CURRENT_FORMAT_HAS_NO_AS_OF" in snapshot.warnings
...
def test_adapter_does_not_invent_quantity_or_price(portfolio_raw):
    snapshot = load_current_portfolio(portfolio_raw)
    assert "WEIGHTS_ARE_DECLARED_PERCENTAGES_NOT_QUANTITY_X_PRICE" in snapshot.warnings
